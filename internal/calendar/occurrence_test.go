...
func TestResolveDayFirstClassSuppressesLowRankCommemorations(t *testing.T) {
	m := ComputeMoveableDates(2026)
	date := time.Date(2026, 4, 13, 0, 0, 0, 0, time.UTC)

	easterMonday := &models.Feast{
		ID:       "easter-monday",
		Name:     "Easter Monday",
		Rank:     models.Double1stClass,
		Color:    models.White,
	...
	// Ash Wednesday is also 1cl but still commemorates ordinary saints.
	ashWednesday := &models.Feast{
		ID:       "ash-wednesday",
		Name:     "Ash Wednesday",
		Rank:     models.Double1stClass,
		Color:    models.Violet,
	...
	hermengild := &models.Feast{
		ID:       "comm-04-13-st-hermengild-martyr",
		Name:     "St Hermengild, Martyr",
		Rank:     models.Commemoration,
		Color:    models.Red,
		Category: models.CategoryMartyr,
		Month:    4,
	...
	walburga := &models.Feast{
		ID:       "comm-02-25-st-walburga",
		Name:     "St. Walburga of Heidenheim, Virgin",
		Rank:     models.Commemoration,
		Color:    models.White,
		Category: models.CategoryVirgin,
		Month:    2,
		Day:      25,
	}
	sunday := &models.Feast{
	...
	feria := &models.Feast{
		ID:       "privileged-lenten-feria",
		Name:     "Wednesday after Lent IV",
		Rank:     models.PrivilegedFeria,
		Color:    models.Violet,
		Category: models.CategoryFeria,
	}
	octaveDay := &models.Feast{
		ID:       "nativity-john-baptist-octave-day-6",
		Name:     "Day VI within the Octave of St John Baptist",
		Rank:     models.SemiDouble,
		Color:    models.White,
		Category: models.CategoryConfessor,
	}
	...
	day, _ := ResolveDay(
		date,
		[]*models.Feast{easterMonday, hermengild, sunday, barnabas, feria, octaveDay},
		models.Easter,
		models.White,
		m,
		nil,
	...
	if day.Celebration != easterMonday {
		t.Fatalf("celebration = %#v, want Easter Monday", day.Celebration)
	}
	...
	if got[hermengild.ID] {
		t.Fatal("expected simple/commemoration saint suppressed on Easter Monday")
	}
	if got[octaveDay.ID] {
		t.Fatal("expected semi-double octave day suppressed on Easter Monday")
	}
	if !got[sunday.ID] {
		t.Fatal("expected Sunday retained on Easter Monday")
	}
	if !got[barnabas.ID] {
		t.Fatal("expected Greater Double retained on Easter Monday")
	}
	if !got[feria.ID] {
		t.Fatal("expected privileged feria retained on Easter Monday")
	}

	ashDay, _ := ResolveDay(
		time.Date(2026, 2, 25, 0, 0, 0, 0, time.UTC),
		[]*models.Feast{ashWednesday, walburga},
		models.Lent,
		models.Violet,
		m,
		nil,
	)
	if ashDay.Celebration != ashWednesday {
		t.Fatalf("celebration = %#v, want Ash Wednesday", ashDay.Celebration)
	}
	if len(ashDay.Commemorations) != 1 || ashDay.Commemorations[0] != walburga {
		t.Fatalf("Ash Wednesday should still commemorate Walburga, got %#v", ashDay.Commemorations)
	}
}
